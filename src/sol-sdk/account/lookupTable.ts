import { AccountInfo, AddressLookupTableAccount, Connection, PublicKey } from '@solana/web3.js';
...
class LookupTable {
  ...
  private updateCache(lutAddress: PublicKey, lutAccount: AddressLookupTableAccount) {
    // console.log('lookupTable log lutAccount', lutAccount);
  ...
  async getLookupTable(
    ...
    // console.log("lutAddressStr", lutAddressStr)
  ...
  computeIdealLookupTablesForAddresses(addresses: PublicKey[]): AddressLookupTableAccount[] {
    ...
    const sortedIntersectionArray = Array.from(tableIntersections.entries()).sort(
      (a, b) => b[1] - a[1]
    );
...
export const lookupTableProvider = (connection: Connection) => {
  return new LookupTable(connection);
};
