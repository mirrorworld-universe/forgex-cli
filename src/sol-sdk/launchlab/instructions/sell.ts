...
export const sellExactInInstruction = async ({
  provider,
  owner,
  baseMint,
  quoteMint,
  ...
  const lamports = new BN(baseLamports).toNumber();
  ...
  transaction.add(
    await program.methods
      .sellExactIn(new BN(amountIn.toString()), new BN(minAmountOut.toString()), new BN(0))
      .accounts({
        payer: owner,
